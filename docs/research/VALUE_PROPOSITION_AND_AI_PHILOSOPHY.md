...
2. **Thermodynamic Ground Truth is Solved by Physics:**  
   Transformer top-oil rise ($\Delta \theta_o$), winding hot-spot rise ($\Delta \theta_w$), and Arrhenius insulation aging factor $V(t)$ can be represented with standards-based physical differential equations from **IEEE Std C57.91-2011** and **IEC 60076-7**. For this prototype, a transparent first-principles model is preferable to an unvalidated black-box surrogate, while field use would still require asset-specific calibration.
...
5. **Mitigation Outcome:** 12-hour recommendation schedule (pre-cooling + 5.0 MW BESS) reducing the modelled hot-spot from $159.53^\circ\mathrm{C}$ to $122.53^\circ\mathrm{C}$ and estimating about \$2.57M in assumption-based avoided exposure.
...
│                                                                                                        │
│  1. 💵 Scenario Financial Value   ──► ~\$2.57M Modeled Avoided Exposure (assumption-based)             │
│  2. ⚡ Grid Reliability & Life     ──► 365.4 Modeled Aging Hours Avoided                               │
...
* **Modeled Avoided Consequence Exposure:** +\$2,565,951
* **Capital-Aging Deferral (365.4 equivalent aging hours avoided):** +\$710
* **Assumed Mitigation Cost (BESS degradation + fan power):** -\$469
* **MODELED AVOIDED EXPOSURE:** **+\$2,566,192.66 per scenario event**
* **ASSUMPTION-BASED COST RATIO:** **5,472.6×**

These outputs use an uncalibrated scenario risk model and disclosed cost assumptions; they are neither realized savings nor an actuarial forecast.
